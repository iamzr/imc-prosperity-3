...
class Status:
    ...
    def cls_update(cls, state: TradingState) -> None:
        ...
        # logger.print(f"Observations: {
        #             state.observations}")

        if MACRONS in state.observations.conversionObservations:
            cls._hist_observation["sugarPrice"].append(
                state.observations.conversionObservations[MACRONS].sugarPrice
            )
            cls._hist_observation["sunlightIndex"].append(
                state.observations.conversionObservations[MACRONS].sunlightIndex
            )
            cls._hist_observation["transportFees"].append(
                state.observations.conversionObservations[MACRONS].transportFees
            )
            cls._hist_observation["exportTariff"].append(
                state.observations.conversionObservations[MACRONS].exportTariff
            )
            cls._hist_observation["importTariff"].append(
                state.observations.conversionObservations[MACRONS].importTariff
            )
            cls._hist_observation["bidPrice"].append(
                state.observations.conversionObservations[MACRONS].bidPrice
            )
            cls._hist_observation["askPrice"].append(
                state.observations.conversionObservations[MACRONS].askPrice
            )
...
class Trade:
    ...
    def kelp(state: Status) -> list[Order]:
        ...
        orders.extend(
            Strategy.mm_ou(
                state=state, fair_price=current_price, gamma=0.1, order_amount=50
            )
        )
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # logger.print(
        #     f"Observations: {
        #              state.observations.conversionObservations[MACRONS]}"
        # )
