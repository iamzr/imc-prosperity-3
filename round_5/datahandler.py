...
class DataHandler:
    ...
    def get_prices(self, round_num: int, day_num: int) -> pd.DataFrame:
        return pd.read_csv(
            pathlib.Path(self.directory)
            / f"prices_round_{round_num}_day_{day_num}.csv",
            sep=";",
        )

    def get_trades(self, round_num: int, day_num: int) -> pd.DataFrame:
        file = (
            pathlib.Path(self.directory) / f"trades_round_{round_num}_day_{day_num}.csv"
        )
        if file.is_file():
